...
int main()
{
    ...
    if (compareDayOfYear(today, birthday))
        ...
        cout << ">>>  Today is NOT your birthday! HAHAHAA!  <<<" << endl << endl;

    cout << "(* There could be +- 1 day, +- 1 month error. *)" << endl << endl;
...
int MonthCalculate(DayOfYear& D1, DayOfYear& D2)                // calculate total months since you were born
{
    ...
    {
        if ((D2.day + monthDays(D1) - D1.day) >= 30)
    ...
    else if (D1.month > D2.month)
    {
        if ((D1.day + 1) >= D2.day)
            if (D2.month == 2)
            {
                if ((D1.day + monthDays(D2) - D2.day) >= 30)
                    months = D1.month - D2.month + 1;////
                else
